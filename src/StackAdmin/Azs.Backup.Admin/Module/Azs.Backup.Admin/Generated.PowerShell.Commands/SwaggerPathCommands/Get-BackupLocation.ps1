...
function Get-BackupLocation
{
    ...
    $skippedCount = 0
    $returnedCount = 0
    if ('BackupLocations_List' -eq $PsCmdlet.ParameterSetName) {
    ...
    } elseif ('BackupLocations_Get' -eq $PsCmdlet.ParameterSetName ) {
    ...
    if ($TaskResult) {
           ...
           $taskResult.Result.Body)
        {
            ...
            if ($result -is [Microsoft.Rest.Azure.IPage[Microsoft.AzureStack.Management.Backup.Admin.Models.BackupLocation]]) {
        ...
        while ($result -and (Get-Member -InputObject $result -Name 'nextLink') -and $result.'nextLink' -and (($Top -eq -1) -or ($returnedCount -lt $Top))) {
            ...
            $taskResult = $BackupAdminClient.BackupLocations.ListNextWithHttpMessagesAsync($result.'nextLink')
             $result = $null
        $ErrorActionPreference = 'Stop'
