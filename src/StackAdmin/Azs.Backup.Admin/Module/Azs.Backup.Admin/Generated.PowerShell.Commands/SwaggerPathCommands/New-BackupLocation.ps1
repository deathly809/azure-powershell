...
#>
function New-BackupLocation
{
    [OutputType([Microsoft.AzureStack.Management.Backup.Admin.Models.BackupLocation])]
    [CmdletBinding(DefaultParameterSetName='BackupLocations_Create')]
    param(    
        [Parameter(Mandatory = $true, ParameterSetName = 'BackupLocations_Create')]
    ...
    $skippedCount = 0
    $returnedCount = 0
    if ('BackupLocations_Create' -eq $PsCmdlet.ParameterSetName) {
        Write-Verbose -Message 'Performing operation CreateWithHttpMessagesAsync on $BackupAdminClient.'
        $taskResult = $BackupAdminClient.BackupLocations.CreateWithHttpMessagesAsync($ResourceGroup)
    } elseif ('BackupLocations_CreateBackup' -eq $PsCmdlet.ParameterSetName ) {
