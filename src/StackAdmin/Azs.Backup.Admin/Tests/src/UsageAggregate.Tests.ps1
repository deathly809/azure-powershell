...
<#
.SYNOPSIS
    Run AzureStack Backup admin edge gateway tests.

.DESCRIPTION
    Run AzureStack Backup admin edge gateway tests using either mock client or our client.
...
InModuleScope Azs.Backup.Admin {
	...
	Describe "SubscriberUsageAggregates" -Tags @('SubscriberUsageAggregate', 'Azs.Backup.Admin') {
