...
function Get-AzsStorageAcquisition
{
    ...
    if ('Acquisitions_List' -eq $PsCmdlet.ParameterSetName) {
        ...
        $TaskResult = $StorageAdminClient.Acquisitions.ListWithHttpMessagesAsync($ResourceGroupName, $FarmId, $(if ($PSBoundParameters.ContainsKey('Filter')) { $Filter } else { [NullString]::Value }))
    } else {
